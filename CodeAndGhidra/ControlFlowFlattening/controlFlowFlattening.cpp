...
bool checkPassword(const std::string& userInput) {
    ...
    while (true) {
        ...
        switch (state) {
            ...
            case 1:
                // State 1: Loop and check characters
                if (i < correctPassword.length()) {
                    // We throw in a little bit of instruction subs
                    unsigned char a = static_cast<unsigned char>(userInput[i]);
                    unsigned char b = static_cast<unsigned char>(correctPassword[i]);
                    unsigned char noiseA = static_cast<unsigned char>((a ^ 0xAA) ^ 0xAA);
                    unsigned char noiseB = static_cast<unsigned char>((b + 13) - 13);
